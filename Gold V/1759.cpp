...
bool in_gather(char x[]) {
    for(int i=0;i<L;i++) {
        if(is_gather(x[i])) return true;
    }
    return false;
}

bool is_consonant(char x) {
    if(x=='a' || x=='e') return false;
    else if(x=='i' || x=='o') return false;
    else if(x=='u') return false;
    return true;
}
